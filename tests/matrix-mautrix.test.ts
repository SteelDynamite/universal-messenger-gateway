...
test("default Matrix factory can select the mautrix sidecar implementation", async () => {
  ...
  expect(await provider.listChats()).toEqual([
    {
      ...
      displayName: "Room",
      type: "direct",
      avatarUrl: "mxc://example/room-avatar",
    },
  ]);
  expect(await provider.getChat("!room")).toEqual({
    chatId: "!room",
    displayName: "Room",
    type: "direct",
    avatarUrl: "mxc://example/room-avatar",
  });
  expect(await provider.listMembers("!room")).toEqual([
    { userId: "@alice:example", displayName: "Alice" },
  ]);
  expect(await provider.getRelations("!room", "$history")).toMatchObject({
    message: { messageId: "$history" },
    items: [{ messageId: "$reaction", relationType: "m.annotation" }],
    nextCursor: "relations-next",
    hasMore: true,
  });
  expect(await provider.getPinnedMessages("!room")).toEqual([
    {
      messageId: "$pinned",
      status: "available",
      message: {
  ...
  expect(
    await provider.resolveThreadContext({
      transport: "matrix",
      chatId: "!room",
      threadRootId: "$root",
  ...
  expect(messages[0]).toMatchObject({
    ...
    mentionedUserIds: ["@other-bot:example"],
  });
...
async function writeFakeSidecar(dir: string): Promise<string> {
  ...
  await writeFile(
    path,
    `import { appendFileSync } from "node:fs";\nimport { createInterface } from "node:readline";\nconst log = process.env.UMG_FAKE_MAUTRIX_LOG;\nfunction send(value) { process.stdout.write(JSON.stringify(value) + "\\n"); }\nfor await (const line of createInterface({ input: process.stdin })) {\n  const command = JSON.parse(line);\n  if (log) appendFileSync(log, JSON.stringify(command) + "\\n");\n  if (command.type === "connect") {\n    send({ id: command.id, ok: true, result: { userId: "@bot:example" } });\n    send({ type: "message", message: { transport: "matrix", chatId: "!room", content: "from sidecar", timestamp: 1, isGroupChat: false, wasMentioned: false, mentionedUserIds: ["@other-bot:example"] } });\n    send({ type: "reaction", reaction: { transport: "matrix", chatId: "!room", messageId: "$event", reaction: "👍", timestamp: 1 } });\n    send({ type: "typing", typing: { transport: "matrix", chatId: "!room", userIds: ["@alice:example"], observedAt: 1 } });\n    send({ type: "invite", invite: { inviteId: "!invite", inviter: "@a:example" } });\n    send({ type: "error", category: "matrix-decryption", eventId: "$encrypted", error: "failed" });\n  } else if (command.type === "list_chats") {\n    send({ id: command.id, ok: true, result: [{ chatId: "!room", displayName: "Room", type: "direct", avatarUrl: "mxc://example/room-avatar" }] });\n  } else if (command.type === "get_chat") {\n    send({ id: command.id, ok: true, result: command.chatId === "!room" ? { chatId: "!room", displayName: "Room", type: "direct", avatarUrl: "mxc://example/room-avatar" } : null });\n  } else if (command.type === "list_invites") {\n    send({ id: command.id, ok: true, result: [{ inviteId: "!invite" }] });\n  } else if (command.type === "list_members") {\n    send({ id: command.id, ok: true, result: [{ userId: "@alice:example", displayName: "Alice" }] });\n  } else if (command.type === "get_pinned_messages") {\n    send({ id: command.id, ok: true, result: [{ messageId: "$pinned", status: "available", message: { transport: "matrix", chatId: "!room", messageId: "$pinned", content: "pinned", timestamp: 2 } }, { messageId: "$missing", status: "missing" }, { messageId: "$redacted", status: "redacted" }, { messageId: "$encrypted", status: "undecryptable" }] });\n  } else if (command.type === "get_relations") {\n    send({ id: command.id, ok: true, result: { message: { transport: "matrix", chatId: "!room", messageId: "$history", content: "history match", timestamp: 2 }, items: [{ messageId: "$reaction", relationType: "m.annotation", eventType: "m.reaction", timestamp: 3, userId: "@alice:example", key: "👍" }], nextCursor: "relations-next", hasMore: true } });\n  } else if (command.type === "health") {\n    send({ id: command.id, ok: true, result: [{ category: "matrix-e2ee", status: "ready", summary: "fake ready" }] });\n  } else if (command.type === "search_history") {\n    send({ id: command.id, ok: true, result: { messages: [{ transport: "matrix", chatId: "!room", messageId: "$history", content: "history match", timestamp: 2, permalink: "https://matrix.to/#/!room/%24history", attachments: [{ mediaId: "mxc://example/history", kind: "image", fileName: "history.png" }] }], scannedChats: 1, scannedMessages: 7, skippedDecryption: 2, partial: true, errors: ["search returned partial results at deadline"] } });\n  } else if (command.type === "resolve_thread_context") {\n    send({ id: command.id, ok: true, result: { root: { status: "available", wasMentioned: true, message: { transport: "matrix", chatId: "!room", messageId: "$root", content: "@bot root", timestamp: 1 } }, history: { messages: [{ transport: "matrix", chatId: "!room", messageId: "$root", content: "@bot root", timestamp: 1 }, { transport: "matrix", chatId: "!room", messageId: "$invocation", content: "thread reply", timestamp: 2 }], statuses: ["complete"] } } });\n  } else { \n    send({ id: command.id, ok: true });\n  }\n}\n`,
  );
  return path;
}
